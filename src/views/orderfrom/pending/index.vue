<template>
  <div>
    ...
    <el-card>
      <el-popconfirm title="你是否要批量发货？" confirmButtonText="发货" cancelButtonText="取消" confirmButtonType="success"
              cancelButtonType="text" icon="Van" iconColor="#07c160"  @confirm="batchdelivery">
              <template #reference> <el-button v-has="`btn.pending.driver`" type="success" style="margin-bottom: 10px;" >批量发货</el-button></template>
            </el-popconfirm>
     
      <el-table ref="reftable" border :data="dingdandetail" style="margin-bottom: 10px;">
        ...
        <el-table-column label="商品信息">
          <template #="{ row }">
            <div v-for="item in row.shopcart">{{ item.shopname }}x{{ item.count }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" fixed="right" width="200px">
          <template #="{ row }">
            <el-popconfirm title="你是否要发货？"  confirmButtonText="发货" cancelButtonText="取消" confirmButtonType="success"
              cancelButtonType="text" icon="Van" iconColor="#07c160"  @confirm="delivergoods([row.dingdanid])">
              <template #reference><el-button v-has="`btn.pending.driver`" title="发货" icon="Van" 
                  type="success"></el-button></template>
            </el-popconfirm>
            <el-button v-has="`btn.pending.update`" title="编辑" icon="Edit" type="success" @click="updateorderfrom(row)"></el-button>
            <el-button title="查看详情" @click="lookoutdetail(row)" icon="InfoFilled" type="success"></el-button>
          </template>

        </el-table-column>
      </el-table>
      <el-pagination @size-change="sizeChange" @current-change="getdingdandetail" v-model:currentPage="pageNO"
      ...
      <template #default>
        <el-form label-width="100px">
          ...
          <el-form-item label="商品信息：">
            <el-tag style="margin-top: 5px; margin-right: 5px;" v-for="item in temdrawer.shopcart" type="success">
    ...
    <el-drawer v-model="drawer1" title="订单详情">
      <template #default>
        ...
        <el-row>
          ...
          <el-col :span="20">{{ proxy?.$dayjs(temdrawer.fukuantime).format('YY-MM-DD HH:mm:ss') }}</el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          ...
          <el-tag style="margin:5px 5px 0px 0;" v-for="(item, index) in temdrawer.shopcart" type="success"
            @click="toimg(index)">{{ item.shopname }}x{{ item.count }}</el-tag>
        </el-row>
        <el-carousel ref="refcarousel" :interval="4000" type="card" height="200px">
...
import { SaveOrderfrom, DeliverGoods, SearchOrderFromInfo } from '@/api/orderfrom/pending'
