<template>
  <div>
    ...
    <el-card>
      <el-button icon="Plus" type="success" style="margin: 10px 0;" @click="addspu" :disabled="!AttrStore.Selectvalue">
        添加SPU
      </el-button>
      <el-table border :data="spudetail" style="margin: 10px 0;" >
        ...
        <el-table-column label="SPU操作">
          <template #="{row}">
            <el-button icon="Plus" color="#07c160"  title="增加sku" @click="addsku(row)"></el-button>
            <el-button icon="Edit"  color="#07c160" title="编辑spu" @click="editspu(row)"></el-button>
            <el-button icon="View"  color="#07c160" title="查看sku" @click="tolooksku(row)"></el-button>
            <el-popconfirm title="确定要删除这个属性值吗?" @confirm="deletespu(row.id)" icon="Delete" >
            <template #reference>
              <el-button icon="Delete"  color="#07c160" title="删除spu" >
              </el-button>
            </template>
            </el-popconfirm>
      ...
      <el-pagination layout="prev, pager, next, jumper,->,sizes,total" @current-change="handleCurrentChange" @size-change="handleSizeChange" :page-sizes="[3,6,9]"  :total="total" :page-size="pageSize" v-model:current-page="pageNo"   />
    </el-card>
  </template>
...
const getspudetail=async(pager=1)=>{
  try{
    pageNo.value = pager;
    let  result= await GetSpuDetail(AttrStore.Selectvalue,pageNo.value,pageSize.value)
    if(result.data){
      // console.log(result)
