    ...
    <div>
        <el-card shadow="always" >
            <el-form label-width="100px" size="normal" :model="skudetail">
                <el-form-item label="SKU名称"><el-input placeholder="请输入SKU名称"
                        v-model="skudetail.skuname"></el-input></el-form-item>
                <el-form-item label="价格"><el-input type="number" placeholder="请输入SKU价格"
                        v-model="skudetail.price"></el-input></el-form-item>
                <el-form-item label="重量"><el-input placeholder="请输入SKU重量"
                        v-model="skudetail.weight"></el-input></el-form-item>
                <el-form-item label="SKU描述"><el-input type="textarea" placeholder="请输入SKU描述"
                        v-model="skudetail.skudescription"></el-input></el-form-item>
                <el-form-item label="数量"><el-input type="number" placeholder="请输入SKU数量"
                        v-model="skudetail.skucount"></el-input></el-form-item>
                <el-form-item label="平台属性" size="normal">
                    <div v-for="item in platformattributList">
                        ...
                        <el-select  v-model="item.platformattributidandvalue" >
                            <el-option v-for="item1 in item.value" :label="item1.valuename" :value="`${item.id}:${item1.valuename}`"></el-option>
                        </el-select>
                    </div>
                </el-form-item>
                <el-form-item label="销售属性" size="normal">
                    <div v-for="item in ExistingattributeList">
                        {{item.saleattrname}}
                        <el-select  v-model="item.platformattributidandvalue" >
                            <el-option v-for="item1 in item.saleattrvaluelist" :label="item1" :value="`${item.id}:${item1}`"></el-option>
                        </el-select>
                    </div>
                </el-form-item>
                <el-form-item label="图片名称" size="normal">
                    <el-table border stripe :data="SpuImageList" @select="setectiondefaultimg" ref="table" :header-cell-class-name="cellClass">
                        ...
                        <el-table-column label="操作" >
                            <template #="{row}">
                            <el-button type="success" @click="setdefaultimg(row)">设置为默认图片</el-button>
                        </template>
                        </el-table-column>
                    </el-table>
...
import { GeteExistingattribute, GetSpuImage,SetSkuDetail } from '@/api/product/spu'
...
const getskuinfo = async (Selectvalue: number, spuid: number) => {
    ...
    console.log(ExistingattributeList.value, platformattributList.value, SpuImageList.value)
...
const setskuindfo= async()=>{
  ...
  ,[]);
  skudetail.skuplatformattributeList=platformattribut.reduce((prev:any,next:any)=>{
