        ...
        <el-card>
            <el-form label-width="110" :model="spudetail">
                ...
                <el-form-item label="SPU销售属性 :">
                    ...
                    <el-button icon="Plus" @click="addsaleattrbuite" :disabled="values == ''">添加属性</el-button>
                            ...
                            <template #="{ row, $index }" style="display: flex;">
                                <div :key="$index" style="display: inline-block;">
                                    <el-tag style="margin-left: 10px;" type="success" size="small" closable
                                        v-for="(item, index) in  row.saleattrvaluelist " @close="removevalue(index, row)"
                                        :key="index">
                                        {{ item }}
                                    </el-tag>
                                </div>
                                <div style="display: inline-block;margin-left: 10px;">
                                    <el-button v-if="!row.flag" type="success" size="small" 
                                        @click="addsaleattrbuitevalue(row, $index)" icon="Plus"></el-button>
                                    <el-input v-else v-model="row.saleattr" placeholder="" size="normal"
                                        :ref="(vc: any) => (inputArr[$index] = vc)" clearable
                                        @blur="saveaddvalue(row)"></el-input>
                                </div>
                            </template>
                        </el-table-column>
                ...
                <el-button type="success" size="default" @click="save"
                    :disabled="!spudetail.spuSaleAttrList?.length">保存</el-button>
                <el-button type="success" size="default" @click="exit">取消</el-button>
            </el-form>
        </el-card>
    </div>
</template>
...
import { GetSaleattr, GeteExistingattribute, SetSpuDetail, GetSpuImage } from '@/api/product/spu'
...
const SaleAttr = computed(() =>
    saleattrList.value.filter((saleattr: any) => {
        ...
        return !spudetail.value.spuSaleAttrList?.find(
            spuSaleAttrlist => {
...
const addsaleattrbuite = () => {
    SaleAttr.value.find((attr: any) => { return attr.basesaleattrid == values.value }).spuid = spudetail.value.id
    if (spudetail.value.spuSaleAttrList) {
        spudetail.value.spuSaleAttrList?.push(SaleAttr.value.find((attr: any) => { return attr.basesaleattrid == values.value }))
        values.value = ''
    } else {
        console.log(SaleAttr.value)
        spudetail.value.spuSaleAttrList = ([SaleAttr.value.find((attr: any) => { return attr.basesaleattrid == values.value })])
        values.value = ''
    }
...
const deletevalue = (row: any) => {
    ...
    spudetail.value.spuSaleAttrList = spudetail.value.spuSaleAttrList?.filter((attrlist) => {
