<template>
  <div>
    ...
    <el-dialog
      :title="addrolevalue.id == 0 ? '添加角色' : '更新角色'"
      v-model="dialog"
      width="30%"
    >
...
const searchrole = async () => {
  const result = await SearchRole(searchrolevalue.value)
  if (result.code == 200) {
    roledetail.value = result.data
    total.value = result.total
